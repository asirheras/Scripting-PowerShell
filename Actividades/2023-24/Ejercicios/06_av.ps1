...
#Definimos las variables que usara el script: En este caso es $archivos, que sera de tipo cadena (string) y 
...
if (-not (Test-Path $carpetaDocuments -PathType Container)) {
    Write-Host "La carpeta Documents no se encuentra en el perfil de usuario."
    exit
}
...
# Itera a traves de cada archivo y agrega su contenido al resultado
...
Write-Host "¡¡Archivos combinados con EXITO!!. El resultado se guardara en: $rutaResultado"
